...
- 교수님 답변 대비는 질문 아래에 30초 답변을 바로 보여줍니다. `질문 언어`와 `답변 언어`에서 각각 English·한국어를 선택할 수 있으며, 선택은 이 브라우저에서 다른 논문으로 이동하거나 다시 방문해도 유지됩니다. 기본은 영어 질문·영어 답변입니다.
...
- 번역본에서 마우스를 올리거나 키보드 초점을 옮기면 해당 문장만 강조합니다. 문장을 클릭·탭하거나 Enter·Space로 선택하면 바로 아래에 영어 원문이 펼쳐집니다. 한 번에 한 문장을 열며, 다시 누르거나 Esc·바깥 클릭으로 닫습니다. 원문을 읽거나 복사하는 동안에는 열린 상태를 유지합니다.
