...
- 퀴즈 진행과 오답은 이 브라우저에 저장되며 이어 풀거나 저장 오답만 복습할 수 있습니다. 문항이 바뀌면 이전 퀴즈 기록은 무효화됩니다. 교수님 답변 연습은 먼저 자기 답안을 쓰고 30초 모델 답변을 펼쳐 비교하는 방식이며 자동 채점하지 않습니다.
- 본문·번역본에서는 글자 크기, 북마크, 중요 부분 표시와 읽던 위치 복원을 지원합니다. 홈의 읽기 순서는 manifest 배열 순서이며, 날짜로 고른 가장 가까운 읽기는 `다음 읽기`로 표시합니다.
...
기본 검사는 manifest의 전체 읽기를 대상으로 합니다. 첫 두 편만 확인하는 명령에는 `:pilot`이 붙습니다. `npm run check:app`은 실제 DOM에서 출제→입력→채점→결과→재풀이와 저장·복원, 읽기 도구, 답변 연습을 실행합니다. `check:validation`과 `check:build`는 승인 의존 파일 변경, 잘못된 정렬, 누락·오래된 생성물, 초안 공개 차단과 manifest 순서의 회귀를 검사합니다.
