...
const SLUG = "gruenewald-et-al-2016";
...
// Monotonic [Korean indexes, source indexes] used only where translation and
// source require different sentence boundaries. Every rule is reviewed against
// the complete bilingual paragraph before inclusion.
const GROUP_RULES = {
  "gruenewald-et-al-2016-tr-006": [
    [[0, 1], [0, 1]],
  ],
  "gruenewald-et-al-2016-tr-023": [
    [[0, 1, 2, 3, 4, 5, 6], [0]],
    [[7], [1]],
    [[8], [2]],
    [[9, 10], [3]],
...
function reviewedSentenceSplit(value) {
  const replacements = [
    ...
    ["D.C.", "DCABBREVIATIONTOKEN"],
    ["Corp.", "CORPABBREVIATIONTOKEN"],
...
function main() {
  ...
  if (resolved.entries.length !== 87) {
    throw new Error(`${SLUG}: expected 87 aligned paragraphs, found ${resolved.entries.length}`);
  }
  ...
  alignment.sentence_alignment_note =
    `All ${pairCount} sentence pairs were checked against the complete bilingual paragraph blocks for meaning, order, omissions, citations, names, numbers, qualification language, EC/BECT design, ITT and CACE estimands, compliance and exposure definitions, assumptions, dose-response interpretation, and source fidelity. Sentence-boundary mismatch blocks were regrouped manually; all 47 references remain verbatim.`;
