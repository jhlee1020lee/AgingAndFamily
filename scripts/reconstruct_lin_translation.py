...
HEADING_TRANSLATIONS = {
    "# Antecedents of Gray Divorce: A Life Course Perspective": "# 황혼이혼의 선행요인: 생애과정 관점",
    "## Abstract": "## 초록",
    "## Keywords": "## 핵심어",
    "## Background": "## 배경",
    "### A Life Course Perspective on Gray Divorce": "### 황혼이혼에 대한 생애과정 관점",
    "### Demographic Predictors of Gray Divorce": "### 황혼이혼의 인구사회학적 예측요인",
    "#### Marital biography": "#### 결혼이력",
    "#### Marital quality": "#### 결혼의 질",
    "#### Spousal homogamy and shared economic resources": "#### 배우자 동질혼과 공동 경제자원",
    "## The Current Study": "## 본 연구",
    "## Method": "## 방법",
    "### Measures": "### 측정",
    "#### Three key life-course transitions": "#### 세 가지 핵심 생애과정 전환",
    "#### Spousal homogamy": "#### 배우자 동질혼",
    "#### Shared economic resources": "#### 공동 경제자원",
    "### Analytic Strategy": "### 분석 전략",
    "## Results": "## 결과",
    "### Bivariate Results": "### 이변량 결과",
    "### Multivariate Results": "### 다변량 결과",
    "## Discussion": "## 논의",
    "## Funding": "## 연구비 지원",
    "## References": "## 참고문헌",
}
...
IMAGE_ALT_TRANSLATIONS = {
    "Figure 1. Cumulative probability of gray divorce by marriage order": "그림 1. 결혼차수별 황혼이혼 누적확률",
    "Table 1. Weighted means or percentages of couples' characteristics at baseline": "표 1. 기초선 부부 특성의 가중평균 또는 백분율",
    "Table 2. Odds ratios from discrete-time logistic regressions of gray divorce": "표 2. 황혼이혼에 대한 이산시간 로지스틱 회귀분석의 승산비",
}
...
def build_translation(source: str) -> str:
    ...
    if len(source_blocks) != 121:
        raise ValueError(f"Expected 121 source blocks, found {len(source_blocks)}")
    ...
    if translation_index != 58 or translation_index != len(TRANSLATED_BLOCKS):
        raise ValueError(
            f"Expected 58 translated blocks; consumed {translation_index}, prepared {len(TRANSLATED_BLOCKS)}"
        )
    ...
    if len(references) != 36:
        raise ValueError(f"Expected 36 source references, found {len(references)}")
...
def main() -> None:
    ...
    print(
        ...
        f"{len(IMAGE_ALT_TRANSLATIONS)} translated image alts, 36 references preserved"
    )
