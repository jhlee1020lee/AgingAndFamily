...
HEADING_TRANSLATIONS = {
    "# Turning points in long distance grandparent–grandchild relationships": "# 장거리 조부모–손자녀 관계의 전환점",
    "## Abstract": "## 초록",
    "## Keywords": "## 핵심어",
    "## Introduction": "## 서론",
    "### Grandparenting": "### 조부모 역할",
    "### Long distance grandparenting": "### 장거리 조부모 역할",
    "### Turning points in grandparenting relationships": "### 조부모 관계의 전환점",
    "### Rationale and research questions": "### 연구 근거와 연구질문",
    "## Method": "## 방법",
    "### Participants": "### 참여자",
    "### Procedure": "### 절차",
    "### Data analysis": "### 자료분석",
    "## Results": "## 결과",
    "## Discussion": "## 논의",
    "## Limitations and conclusions": "## 한계와 결론",
    "## References": "## 참고문헌",
}


IMAGE_ALT_TRANSLATIONS = {
    "Figure 1. Decrease in relational closeness": "그림 1. 관계적 친밀감 감소",
    "Figure 2. Consistent relational closeness": "그림 2. 일관된 관계적 친밀감",
    "Figure 3. Minimal changes in relational closeness": "그림 3. 관계적 친밀감의 최소 변화",
    "Figure 4. Multidimensional changes in relational closeness": "그림 4. 관계적 친밀감의 다차원적 변화",
    "Figure 5. Increase in relational closeness": "그림 5. 관계적 친밀감 증가",
    "Table 1. Turning point frequency, relational impact, and definition": "표 1. 전환점의 빈도, 관계에 미친 영향 및 정의",
}
...
def build_translation(source: str) -> str:
    ...
    if translation_index != 85 or translation_index != len(TRANSLATED_BLOCKS):
        raise ValueError(
            f"Expected exactly 85 translated non-reference blocks, consumed {translation_index}, "
    ...
    for index, (original, translated) in enumerate(
        ...
        source_counter = digit_tokens(original)
        translated_counter = digit_tokens(translated)
        # Natural Korean large-number renderings retain the same quantities.
        if index == 10:
            if "5,600만" in translated:
                source_counter["56"] -= 1
            if "1,950만" in translated:
                source_counter["19.5"] -= 1
            source_counter += Counter()
        missing_counter = source_counter - translated_counter
        missing = [
    ...
    if reference_count != 31:
        raise ValueError(f"Expected 31 source references, found {reference_count}")
    if source_refs != translated_refs:
        raise ValueError("Reference section changed during translation reconstruction")

    required = (
        "관계적 친밀감의 뚜렷한 감소(N = 10)",
        "일관된 관계적 친밀감(N = 8)",
        "관계적 친밀감의 뚜렷한 증가(N = 2)",
        "1 — 가장 가까움; 5 — 가장 멂",
        "면접 5건은 전사할 수 없었다",
        "참여자 35명(N = 35)",
        "60–82세(M = 70.9, SD = 5.34)",
        "12–19세(M = 14.7, SD = 2.36)",
        "참여자 22명의 보고만으로 실질적인 변화 사건 100개가 넘게",
        "합계: 빈도 100, 관계에 미친 긍정적 영향 50, 관계에 미친 부정적 영향 50",
        "P11–P20의 궤적 10개",
        "P9와 P10의 궤적 2개",
        "지난 3년 동안은 그 애 혼자",
        "400마일",
    ...
    forbidden = re.compile(
        r"(?:chatbot|chatgpt|\bstt\b|lecture\s+audio|학생\s*정보|수강생\s*정보|"
...
def main() -> None:
    ...
    print(
        ...
        f"{len(IMAGE_ALT_TRANSLATIONS)} translated image alts, 31 verbatim references"
    )
