...
HEADING_TRANSLATIONS = {
    "# Changing Sibling Relationships After Parents’ Death: The Role of Solidarity and Kinkeeping": "# 부모 사망 후 형제자매 관계의 변화: 결속과 친족유지의 역할",
    "## Abstract": "## 초록",
    "## Key Words": "## 핵심어",
    "## Background": "## 배경",
    "### Previous Research": "### 선행연구",
    "### Theoretical Background and Hypotheses": "### 이론적 배경과 가설",
    "## Method": "## 방법",
    "### Data and Sample": "### 자료와 표본",
    "### Measures": "### 측정",
    "#### Dependent variables": "#### 종속변수",
    "#### Independent variables": "#### 독립변수",
    "#### Mediator variables": "#### 매개변수",
    "#### Control variables": "#### 통제변수",
    "### Models": "### 모형",
    "## Results": "## 결과",
    "## Conclusion and Discussion": "## 결론 및 논의",
    "## Note": "## 주",
    "## References": "## 참고문헌",
}
...
IMAGE_ALT_TRANSLATIONS = {
    "Table 1. Descriptive statistics": "표 1. 기술통계",
    "Table 2. Frequency of sibling contact and conflict in all waves for all dyads": "표 2. 모든 조사차수·모든 다이애드의 형제자매 접촉과 갈등 빈도",
    "Table 3. Fixed effects regression of changes in contact with siblings": "표 3. 형제자매 접촉 변화의 고정효과 회귀분석",
    "Figure 1. First parent’s death and changes in sibling contact": "그림 1. 첫 번째 부모 사망과 형제자매 접촉 변화",
    "Figure 2. Second parent’s death and changes in sibling contact": "그림 2. 두 번째 부모 사망과 형제자매 접촉 변화",
    "Table 4. Fixed effects logit and linear probability regression of changes in conflict with siblings": "표 4. 형제자매 갈등 변화의 고정효과 로짓 및 선형확률 회귀분석",
}
...
def build_translation(source: str) -> str:
    ...
    if reference_count != 52:
        raise ValueError(f"Expected 52 source references, found {reference_count}")
...
def main() -> None:
    ...
    print(
        ...
        f"{len(IMAGE_ALT_TRANSLATIONS)} translated image alts"
    )
