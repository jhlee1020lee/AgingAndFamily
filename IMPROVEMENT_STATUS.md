...
번역본 원문 표시: 마우스 올리기·초점 이동으로 원문을 자동 표시하던 동작을 없앴다. 한국어 문장을 클릭·탭하거나 키보드 Enter·Space로 선택하면 해당 문장 아래에 영어 원문이 문서 흐름을 따라 펼쳐져 번역문을 덮지 않는다. 다시 선택·Esc·바깥 클릭으로 닫고, 원문 안의 클릭·초점 이동·마우스 이탈로는 닫지 않는다. 원문·번역·정렬 데이터는 변경하지 않는다.

원문 표시 검증: 전체 빌드 후 Node 20의 `npm run check`를 통과했다. 22개 번역본의 6,664개 문장 원문 영역과 접근성 연결, 클릭·키보드·터치 동작을 검사했다. 실제 브라우저의 1280px·390px 화면에서 한국어 아래로 원문이 배치되고 다음 한국어 문장도 원문 아래로 이동하는 것을 확인했다. 마우스를 올릴 때는 닫힌 상태를 유지했으며 Enter·Space·Esc와 밝은 화면·다크 모드를 확인했다. 기존 콘텐츠 승인 해시는 유지하며 메타데이터는 빌드 검증 시각만 갱신했다.

질문·답변 언어 선택: 교수님 답변 대비에서 질문과 답변을 각각 English·한국어로 고를 수 있다. 기본은 영어이며 두 선택은 다른 논문으로 이동하거나 다시 방문해도 유지한다. 한국어 질문과 완전한 답변은 같은 카드의 `title_ko`·`answer_30s_ko`에 저장하고, 기존 영어·카드 ID·근거 ID는 보존한다. 번역은 승인 해시 및 생성된 문구의 일치 검사에 포함한다.

언어 선택 검증: 22편의 462개 질문·답변을 모두 번역하고 원문과 대조한 뒤 해당 답변 대비 페이지의 승인 해시를 갱신했다. 전체 빌드와 Node 20의 `npm run check`를 통과했으며, 네 가지 언어 조합과 선택 저장·복원, 기존 퀴즈 990문항을 확인했다. 실제 브라우저의 데스크톱 1280px·모바일 390px에서 언어 전환과 논문 간 설정 유지를 확인했고 가로 넘침은 없었다.

이전 화면 조정: 사용자 요청에 따라 본문·번역의 글자 크기, 북마크, 읽던 위치 저장·복원, 중요 표시 기능을 제거했다. 교수님 답변 대비는 입력창과 답변 펼치기를 없애고 질문 아래에 답변을 바로 표시한다. 이전에 저장된 글자 크기·읽기 위치·연습 초안은 새 화면에 적용하지 않는다. 아래의 과거 검증 기록 중 해당 기능을 확인한 내용은 제거 이전의 기록이다.

이전 화면 조정 검증: 전체 빌드 후 Node 20의 `npm run check`를 통과했다. 44개 본문·번역 페이지의 도구 제거, 462개 답변의 직접 표시, 기존 퀴즈 990문항 동작을 확인했다. 실제 브라우저에서 1280px 본문·답변 준비와 390px 번역·답변 준비 화면을 확인했으며 가로 넘침은 없었다. 당시 manifest와 원문·번역·학습 JSON은 변경하지 않았다.
...
| 구술 답변 대비 | 입력창 없이 질문 아래 30초 답변 즉시 표시, 질문·답변 각각 영어·한국어 선택 |
| 승인 해시·개요 승인·생성물 오류 | 44개 검증 회귀 통과. HTML 비교 기호 보존 및 한국어 번역 변조 검사 포함 |
...
| 전체 기본 검사·영어 필수 gate | Node 20.20.2에서 전체 npm run check 통과. 20개 영어 gate 회귀 포함 |
